...
export default function PurchaseOrderSearchPage() {
  ...
  const fetchOrders = async () => {
    ...
    setOrders(res.data as PO[]);
  };

const handleEdit = async (order: PO) => {
  console.log("res");
  try {
    const res = await api.get(`/purchase-order/${order.poId}`);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold">Search Purchase Orders</h2>
        <div className="space-x-2">
          <Button onClick={handleAdd} variant="outline">Add</Button>
        </div>
      </div>
      <PurchaseOrderTable orders={orders} onEdit={handleEdit} />
    </div>
  );
}
